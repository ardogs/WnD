...
export const MainPageCompanies = () => {
  ...
  return (
    <Flex vertical style={{ flex: 1, width: '100%' }}>
      ...
      <div style={{ textAlign: 'center', marginTop: 40 }} className="no-select">
        <Title level={1} style={{ fontSize: 60 }}><b>{t("dashboard.companies.mainPage.title")}</b></Title>
        <Title level={5} style={{ marginBottom: '5px', marginTop: '50px' }}>{t("dashboard.companies.mainPage.instructions")}</Title>
      </div>

      <Flex style={{ flex: 1, marginTop: '20px' }} justify="center" align="center">
        <Flex gap="large" wrap justify="center">
          <Link to="/companies/edit"><Button shape="round" icon={<EditFilled />} size="large" style={{ boxShadow: boxShadow, fontSize: '18px', height: '60px', ...orange }}>{t("dashboard.invoices.mainPage.button.edit")}</Button></Link>
